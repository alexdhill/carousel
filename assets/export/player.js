(function () {
    ...
    function applyReveal(payload) {
        ...
        (payload.hidden || []).forEach(function (id) {
            setVisibility(id, false);
        });
        (payload.shown || []).forEach(function (id) {
            setVisibility(id, true);
        });
        (payload.animate || []).forEach(function (a) {
            playAnimation(a);
        });
    }

    function slide() {
        return DECK.slides[slideIdx];
    }
    function lastStep() {
        return Math.max(0, (slide().snaps || []).length - 1);
    }
    ...
    function advance() {
        if (step < lastStep()) {
            step += 1;
            applyReveal(slide().forwards[step]);
