...
use crate::commands::{relayout_patches, Command, CommandError, CommandOutput, resolve_canvas_mut};
...
impl Command for SetGroupLayout {
    ...
    fn apply(&self, deck: &mut crate::deck::Deck) -> Result<CommandOutput, CommandError> {
        assert!(!self.element_id.is_empty(), "SetGroupLayout: empty element_id");
        let canvas = resolve_canvas_mut(deck, &self.target)?;
        let prior: crate::deck::style::GroupStyle = {
            let el = canvas.find_element_mut(&self.element_id)
            ...
            let before = gs.clone();
            if let Some(d) = self.direction { gs.direction = d; }
            if let Some(d) = self.distribution { gs.distribution = d; }
            if let Some(a) = self.alignment { gs.alignment = a; }
            before
        };
        canvas.mark_dirty();
    ...
    fn label(&self) -> &'static str { "Set Group Layout" }
...
impl Command for SetGroupScale {
    ...
    fn apply(&self, deck: &mut crate::deck::Deck) -> Result<CommandOutput, CommandError> {
        assert!(!self.element_id.is_empty(), "SetGroupScale: empty element_id");
        ...
        let inverse = SetGroupScale {
            target: self.target.clone(), element_id: self.element_id.clone(), scale: prior,
        };
        Ok(CommandOutput {
            patches: Vec::new(),
            inverse: Box::new(inverse),
    ...
    fn label(&self) -> &'static str { "Scale Group" }
    fn requires_remount(&self) -> bool { true }
}
...
mod tests {
    ...
    fn deck_with_group() -> (Deck, crate::deck::SlideId, crate::deck::ElementId) {
        ...
        let mut a = text_element("ga", "t"); a.geometry.x = 0.0; a.geometry.width = 20.0; a.geometry.height = 10.0;
        let mut b = text_element("gb", "t"); b.geometry.x = 80.0; b.geometry.width = 20.0; b.geometry.height = 10.0;
    ...
    fn set_group_layout_sets_props_and_relayouts() {
        ...
        assert!(out.patches.iter().any(|p| matches!(p, crate::ipc::Patch::SetStyle { .. })));
    ...
    fn set_group_scale_sets_scale_and_inverts() {
        ...
        let out = SetGroupScale {
            target: CanvasTarget::Slide(sid.clone()), element_id: gid.clone(), scale: 2.0,
        }.apply(&mut deck).unwrap();
        let g = deck.slides[&sid].find_element(&gid).unwrap();
        match &g.style { ElementStyle::Group(gs) => assert_eq!(gs.scale, 2.0), _ => panic!() }
        out.inverse.apply(&mut deck).unwrap();
        let g2 = deck.slides[&sid].find_element(&gid).unwrap();
        match &g2.style { ElementStyle::Group(gs) => assert_eq!(gs.scale, 1.0), _ => panic!() }
    }
}
